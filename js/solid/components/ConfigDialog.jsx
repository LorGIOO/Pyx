          ...
          <div class="cfg-main">
            ...
            <div class="cfg-body">
              <Show when={cat() === 'general'}>
                ...
                <div class="cfg-row">
                  <span class="cfg-label">{tr('Guardar los resultados dentro del documento', 'Store results inside the document')}</span>
                  <input type="checkbox" checked={general.saveOutputs !== false}
                    onChange={(e) => setGeneral({ saveOutputs: e.target.checked })} />
                </div>
                <p class="cfg-hint">{tr(
                  'Los resultados de las celdas viajan dentro del .pltx: al reabrir el informe se ven los números, las tablas y las figuras sin volver a ejecutar nada (ni hace falta Python). Un resultado cuya celda haya cambiado desde entonces se descarta y la celda se marca como desactualizada. Las figuras tienen un límite de tamaño para que el documento siga siendo ligero.',
                  'Cell results travel inside the .pltx: reopening the report shows its numbers, tables and figures without re-running anything (Python is not even needed). A result whose cell has changed since is discarded and the cell is marked out of date. Figure data is capped so the document stays light.',
                )}</p>
              ...
              <Show when={cat() === 'lang'}>
                ...
                <div class="cfg-row">
                  <span class="cfg-label">{tr('Idioma de revisión', 'Proofing language')}</span>
                  <select class="cfg-select" value={spellLang()}
                    onChange={(e) => {
                      broadcastSpellRefresh(); // clear the old language's marks now
                      setSpellLang(e.target.value).then(broadcastSpellRefresh);
                    }}>
                    <option value="es">Español</option>
                    <option value="en">English</option>
                  </select>
                ...
                <p class="cfg-hint">
                  {tr(
                    'Hunspell, el mismo motor de Word/LibreOffice. Clic derecho sobre una palabra subrayada para ver sugerencias o agregarla a tu diccionario. Solo revisa el texto: los comandos LaTeX, las matemáticas y las celdas Python se ignoran. Un idioma sin diccionario instalado (public/dict/<código>.aff y .dic) simplemente no subraya nada.',
                    'Hunspell, the same engine as Word/LibreOffice. Right-click an underlined word to see suggestions or add it to your dictionary. It only checks text: LaTeX commands, math and Python cells are ignored. A language with no dictionary installed (public/dict/<code>.aff and .dic) simply underlines nothing.',
                  )}
                </p>
