...
import { dirOf, joinPath, baseName } from '../core/paths.js';
...
async function openHit(hit) {
  ...
  // Forward slashes are what LaTeX sources use and what every OS accepts,
  // Windows included. Rewriting them to backslashes made Ctrl+click on an
  // \input a Windows-only feature — on macOS and Linux the path never resolved.
  let p = hit.path.trim();
  ...
  for (const c of candidates) {
    ...
    if (IMG_EXT.has(ext)) {
      ...
      docStore.openImageTab(url, baseName(c));
    } else if (ext === 'pdf') {
...
export const pathLinks = [
  ...
  EditorView.domEventHandlers({
    ...
    mousedown(e, view) {
      ...
      // Ctrl+click anywhere ELSE = forward search (TeXstudio both ways), with
      // WORD precision: the exact clicked word is selected here in the editor
      // AND highlighted in the PDF at the SyncTeX target — never just the line.
      const line = view.state.doc.lineAt(pos);
      const col = pos - line.from;
      const isW = (c) => c && /[\p{L}\p{M}\p{N}]/u.test(c);
      let a = col, b = col;
      while (a > 0 && isW(line.text[a - 1])) a--;
      while (b < line.text.length && isW(line.text[b])) b++;
      const word = a < b ? line.text.slice(a, b) : '';
      if (word) {
        // Mark the word on the editor side (the PDF side marks it too).
        e.preventDefault();
        view.dispatch({ selection: { anchor: line.from + a, head: line.from + b } });
      }
      // Dynamic import: commands.js → setup.js → link-paths.js would cycle.
      import('./commands.js').then((m) => m.forwardSearch(line.number, word)).catch(() => {});
