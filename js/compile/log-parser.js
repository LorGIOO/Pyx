...
export function parseLatexLog(log, opts = {}) {
  ...
  // The build copies are CLEAN LaTeX (cells removed), so the engine's line
  // numbers shift below every cell. `lineMaps` (build basename → srcLines[])
  // translates them back, so a click in the Problems panel lands on the real
  // source line.
  const lineMaps = opts.lineMaps || null;
  const mapLine = (name, line) => {
    if (!lineMaps || !name || !line) return line;
    const base = String(name).replace(/^"+|"+$/g, '').split(/[\\/]/).pop().toLowerCase();
    const m = lineMaps[base];
    if (!m || !m.length) return line;
    return m[Math.max(0, Math.min(m.length - 1, line - 1))];
  };
  ...
  const flushItem = () => {
    ...
    push({
      severity: item.type,
      file: mapFile(top),
      line: item.line > 0 ? mapLine(top, item.line) : null,
  ...
  for (let n = 0; n < lines.length; n++) {
    ...
    switch (cookie) {
      case Start: {
        ...
        if (fle) {
          push({ severity: 'error', file: mapFile(fle[1]), line: mapLine(fle[1], +fle[2]), message: simplified(fle[3]) });
          break;
        }
        // 1) badboxes
