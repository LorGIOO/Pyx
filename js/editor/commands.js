...
export function insertSnippet(text) {
  ...
  // TeXstudio behaviour: with text selected, the selection drops INTO the
  // snippet's first empty {} group instead of being thrown away — selecting
  // "x+1" and clicking Fracción gives \frac{x+1}{} with the caret in the
  // second group, ready to type the denominator.
  const slot = text.indexOf('{}');
  if (selected && slot >= 0) {
    const inserted = text.slice(0, slot + 1) + selected + text.slice(slot + 1);
    // Caret lands in the NEXT empty {} when the snippet has more groups to
    // fill; otherwise right after the snippet.
    const nextSlot = inserted.indexOf('{}', slot + 1 + selected.length);
    const anchor = sel.from + (nextSlot >= 0 ? nextSlot + 1 : inserted.length);
    v.dispatch({
...
/** Source line for a line reported against a document's .build.tex. */
export function buildLineToSource(realPath, line) {
  return buildToSrcLine(lineMapFor(realPath), line);
}

// Tries the doc's .build.tex name first (what the engine actually read for
// processed files) — translating the source line to its build line — then the
// raw path (unprocessed .tex children compile as-is, lines match).
// `lineNo` (optional) overrides the cursor line — Ctrl+click passes the
// clicked line so the jump matches the mouse, not the previous caret.
// `word` (optional) is the exact word under the click/caret: the PDF then
// highlights THAT word at the target, not just the line.
export async function forwardSearch(lineNo, word) {
  const v = getView();
  ...
  for (const tex of [buildTex, d.path]) {
    try {
      const qLine = tex === buildTex ? srcToBuildLine(map, line) : line;
      const loc = await synctexView(tex, qLine, state.lastPdfPath);
      if (loc && loc.page) {
        state.previewVisible = true;
        const prev = await import('../pdf/preview.js');
        await prev.showPdfLocation(loc.page, loc.x, loc.y, word);
        return;
      }
